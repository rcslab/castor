...
int
__rr_mknod(const char *path, mode_t mode, dev_t dev)
{
    int		    result;
    RRLogEntry     *e;
    ...
    switch (rrMode) {
    case RRMODE_NORMAL:
	return __rr_syscall(SYS_mknod, path, mode, dev);
    case RRMODE_RECORD:
	result = __rr_syscall(SYS_mknod, path, mode, dev);
	e = RRLog_Alloc(rrlog, threadId);
...
__rr_ffclock_getcounter(ffcounter * ffcount)
{
	...
	RRLog_Append(rrlog, e);
	if (result != -1) {
	    logData((uint8_t *) ffcount, (unsigned long)sizeof(ffcounter));
	}
	break;
    case RRMODE_REPLAY:
	e = RRPlay_Dequeue(rrlog, threadId);
...
int
__rr_getdents(int fd, char *buf, int count)
{
    int		    result;
    RRLogEntry     *e;
    ...
    switch (rrMode) {
    case RRMODE_NORMAL:
	return __rr_syscall(SYS_getdents, fd, buf, count);
    case RRMODE_RECORD:
	result = __rr_syscall(SYS_getdents, fd, buf, count);
	e = RRLog_Alloc(rrlog, threadId);
...
__rr_getauid(au_id_t * auid)
{
	...
	RRLog_Append(rrlog, e);
	if (result != -1) {
	    logData((uint8_t *) auid, (unsigned long)sizeof(au_id_t));
	}
	break;
    case RRMODE_REPLAY:
	e = RRPlay_Dequeue(rrlog, threadId);
...
__rr_cap_getmode(u_int * modep)
{
	...
	RRLog_Append(rrlog, e);
	if (result != -1) {
	    logData((uint8_t *) modep, (unsigned long)sizeof(u_int));
	}
	break;
    case RRMODE_REPLAY:
	e = RRPlay_Dequeue(rrlog, threadId);
...
__rr_cap_fcntls_get(int fd, uint32_t * fcntlrightsp)
{
	...
	RRLog_Append(rrlog, e);
	if (result != -1) {
	    logData((uint8_t *) fcntlrightsp, (unsigned long)sizeof(uint32_t));
	}
	break;
    case RRMODE_REPLAY:
	e = RRPlay_Dequeue(rrlog, threadId);
...
__rr_stat(const char *path, struct stat *ub)
{
    ...
    switch (rrMode) {
    case RRMODE_NORMAL:
	return __rr_syscall(SYS_stat, path, ub);
    case RRMODE_RECORD:
	result = __rr_syscall(SYS_stat, path, ub);
	e = RRLog_Alloc(rrlog, threadId);
...
__rr_lstat(const char *path, struct stat *ub)
{
    ...
    switch (rrMode) {
    case RRMODE_NORMAL:
	return __rr_syscall(SYS_lstat, path, ub);
    case RRMODE_RECORD:
	result = __rr_syscall(SYS_lstat, path, ub);
	e = RRLog_Alloc(rrlog, threadId);
